...
class GitFuncs:
    ...
    create_release_branch = partial(
        BashFunc,
        'git checkout -q -b {branch} --no-track origin/{source}',
    )
    delete_remote_branch = partial(
        BashFunc,
    ...
    cherry_pick = partial(
        BashFunc,
        'git cherry-pick -q {sha}',
    )
    commit = partial(
        BashFunc,
        'git commit -q --allow-empty -am "Release {version}"',
    )
    push = partial(
        BashFunc,
    ...
    merge = partial(
        BashFunc,
        'git merge -q --commit --no-ff {branch} -m "Merge, {branch}"'
    )


class GitFlows:

    def __init__(self, version, release_branch_template):
        self.version = version
        self.release_branch = release_branch_template.format(version=version)
    ...
    def make_release_branch(self, release_set):
        ...
        execute_commands(
            ...
            BashFunc(release_set, version=self.version),
            GitFuncs.commit(version=self.version),
            GitFuncs.push(branch=self.release_branch),
        )

    def make_hotfix_branch(self, list_of_commit_sha,
                           release_set):
        execute_commands(
            'Make hotfix branch',
            GitFuncs.fetch(),
